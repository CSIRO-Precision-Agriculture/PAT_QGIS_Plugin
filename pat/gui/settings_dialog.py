...
from util.check_dependencies import  create_snapshots_table, plugin_status
...
class SettingsDialog(BASE, WIDGET):
    ...
    def update_versions(self,level='basic'):
        ...
        df_dep = pd.melt(df_dep, id_vars=['name','type'], value_vars= ['current','path'], var_name='source', value_name='value').dropna()
    ...
    def on_cmdCheckUpdates_clicked(self):
        ...
        df_dep =  plugin_status(level='basic', check_for_updates=True,forced_update=True)
        ver_file = Path(PLUGIN_DIR).joinpath( 'util','snapshot_table.csv')
        create_snapshots_table(ver_file)
        
        if read_setting(PLUGIN_NAME + '/STATUS/UPDATE_AVAILABLE', object_type=bool,default=False) is False:
